...
import { TCreateSchedule } from '../interfaces/schdules.interface';
import { realEstateRepo, scheduleRepo, userRepo } from '../repositories';

export const createScheduleService = async (data: TCreateSchedule, userId: number): Promise<void> => {
    const newDate: number = new Date(data.date).getDay();
    if ((newDate === 0) || (newDate === 6)) {
        throw new AppError('Invalid date, work days are monday to friday', 400);
    };

    const time: number = Number(data.hour.split(':')[0]);
    if ((time < 8) || (time > 18)) {
